...
export class OpenAiBlogDraftService {
  ...
  private buildPromptText = (
    ...
    return [
      ...
      '',
      '요구사항:',
      '- 제목은 내용을 요약하는 문장이 아니라 실제 사람이 블로그에 붙일 법한 제목으로 작성한다.',
      '- 제목에는 날짜, 장소, 함께한 사람, 핵심 추억 등을 자연스럽게 활용할 수 있다.',
      '- 제목에서 모든 장소를 나열하거나 하루 전체를 요약하려고 하지 않는다.',
      '- 제목은 10~25자 정도를 권장한다.',
      '- 제목에는 필요하다면 ㅎㅎ, !, ~ 정도를 자연스럽게 사용할 수 있다.',
      '- 좋은 제목 예시: 수빈이랑 서울 데이트 ❣️ / 오랜만에 수빈이랑 힐링! / 굿즈 사고 한강까지 완벽했던 하루 / 오늘도 추억 하나 추가 :) / 귀여운 것만 잔뜩 보고 온 하루 / 수빈이랑 놀면 시간이 왜 이렇게 빨라',
      ...
      '- 장소별 감정 이모지는 그대로 복사하지 말고 분위기와 감정선을 보조하는 힌트로만 반영한다.',
      '- mood 이모지를 본문에 그대로 복사하지 않는다.',
      '- 분위기에 맞으면 🥹, 😂, 💦, ✨, 💛, 🫶🏻, 🍃, 👀 같은 비슷한 느낌의 이모지를 가끔 사용할 수 있다.',
      '- 이모지는 본문 전체에서 3~5개 이하로만 사용한다.',
      '- 문장 길이와 어미를 다양하게 섞고, 모든 문체를 통일하지 않는다.',
      '- "~했다."만 반복하지 말고 "~했음", "~했다 ㅋㅋ", "~해서 좋았다", "~인 듯", "~인 것 같다", "~였다!" 같은 표현을 상황에 맞게 자연스럽게 섞는다.',
      '- "특히", "또한", "한편", "무엇보다" 같은 연결어를 반복해서 사용하지 않는다.',
      '- 약간의 여운이나 담백한 표현을 사용할 수 있다.',
      '- ㅋㅋ, ㅎㅎ, !, ~, …는 자연스러운 맥락에서만 가끔 사용할 수 있다.',
      '- ㅋㅋ, ㅎㅎ, !, ~, …는 한 장소 설명에 0~2회 정도만 사용하고 남발하지 않는다.',
